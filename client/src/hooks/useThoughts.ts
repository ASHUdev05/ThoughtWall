...
import { thoughtService, type Thought, type User } from '../services/thoughtService';
...
export const useThoughts = (initialFilter: string = "All", roomId?: number) => {
    ...
    const fetchThoughts = useCallback(async (currPage: number, currFilter: string, currRoomId?: number) => {
        setLoading(true);
        try {
            const data = await thoughtService.getAll(currPage, currFilter, currRoomId);
    ...
    const toggleComplete = async (id: number) => {
        ...
        try {
            // Optimistic update
            setThoughts(prev => prev.map(t => t.id === id ? { ...t, completed: !t.completed } : t));
            await thoughtService.update({ ...thought, completed: !thought.completed });
    ...
    return { 
        ...
        addThought, editThought, togglePin, removeThought, toggleComplete, assignUser,
