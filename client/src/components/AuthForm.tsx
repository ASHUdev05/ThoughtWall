...
const AuthForm: React.FC<Props> = ({ onLogin }) => {
    ...
    try {
        ...
        // If the error message is "User not found" (from Spring), show it directly
        setError(err.message || "Something went wrong.");
    }
};

    return (
        <div className="thought-form" style={{maxWidth: '400px', margin: '2rem auto'}}>
            <h2>{isSignup ? "Sign Up" : "Login"}</h2>
            <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:'1rem'}}>
                <input className="thought-input" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} />
                <input className="thought-input" type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} />
                <button className="submit-btn" type="submit">{isSignup ? "Sign Up" : "Login"}</button>
                {error && <p style={{color: 'var(--primary-color)'}}>{error}</p>}
                <p style={{textAlign:'center', cursor:'pointer', fontSize:'0.9rem'}} onClick={() => setIsSignup(!isSignup)}>
