...
const UserProfileView: React.FC<Props> = ({ onLogout }) => {
  ...
  const handleDeleteRoom = async (roomId: number) => {
    if (
      !confirm(
        "Are you sure? This will delete the room and all thoughts in it.",
      )
    )
      return;
    try {
    ...
    >
      ...
      >
        ...
        <div className="profile-section">
          <h4>Owned Rooms</h4>
          {profile.ownedRooms.length === 0 && (
            <p style={{ opacity: 0.6 }}>No rooms owned.</p>
          )}
        ...
        <div className="profile-section">
          ...
          {profile.assignedTasks.length === 0 && (
            <p style={{ opacity: 0.6 }}>No tasks assigned.</p>
          )}
          <ul style={{ listStyle: "none", padding: 0 }}>
