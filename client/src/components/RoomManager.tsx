...
const RoomManager: React.FC<Props> = ({ activeRoomId, onRoomSelect }) => {
  ...
  return (
    ...
    >
      <h3
        style={{ marginTop: 0, fontSize: "1.1rem", color: "var(--text-color)" }}
      >
        Collaborative Rooms
      </h3>
      ...
      >
        {showCreate ? (
          <div style={{ display: "flex", gap: "0.5rem" }}>
            ...
            />
            <button className="tag-btn active" onClick={handleCreate}>
              Save
            </button>
            <button className="tag-btn" onClick={() => setShowCreate(false)}>
              Cancel
            </button>
          </div>
        ) : (
          <button className="tag-btn" onClick={() => setShowCreate(true)}>
            + New Room
          </button>
        )}
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          />
          <button className="tag-btn" onClick={handleJoin}>
            Join
          </button>
        </div>
      </div>
      ...
      {activeRoomId && (
        ...
        >
          Share code{" "}
          <strong>{rooms.find((r) => r.id === activeRoomId)?.code}</strong> to
          invite others.
        </small>
      )}
    </div>
