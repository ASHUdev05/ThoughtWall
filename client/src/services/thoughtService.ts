...
export const thoughtService = {
    getAll: async (page: number = 0, tag: string = "All", roomId?: number): Promise<PageResponse<Thought>> => {
        const params = new URLSearchParams({ page: page.toString(), size: '5' });
        if (tag && tag !== "All") params.append('tag', tag);
        if (roomId) params.append('roomId', roomId.toString());

        const response = await fetch(`${API_BASE}/thoughts?${params.toString()}`, { headers: getHeaders() });
        if (!response.ok) throw new Error('Failed to fetch thoughts');
        return response.json();
    },

    create: async (content: string, tag: string, roomId?: number): Promise<Thought> => {
    ...
    migrateTag: async (oldTag: string, newTag: string = "General"): Promise<void> => {
        ...
        const response = await fetch(`${API_BASE}/thoughts/tags/migrate?${params.toString()}`, {
            method: 'PUT',
            headers: getHeaders(),
        });
...
export const roomService = {
    ...
    getMyRooms: async (): Promise<Room[]> => {
        const response = await fetch(`${API_BASE}/rooms`, { headers: getHeaders() });
        return response.json();
    },
    getMembers: async (roomId: number): Promise<User[]> => {
        const response = await fetch(`${API_BASE}/rooms/${roomId}/members`, { headers: getHeaders() });
        return response.json();
    },
    // NEW: Delete Room
